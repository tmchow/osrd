...
/// Block length for TVM signals (in meters).
...
const ROLLING_STOCK_DECELERATION: [(f64, f64); 9] = [
...
///
/// If the `speed` is higher than 320.0 km/h, the function returns `None`.
...
struct Track<'a> {
    ...
    ///
    /// The applicable speed section for each track section, if any.
    track_sections: Vec<(&'a TrackSection, Direction, Option<&'a SpeedSection>)>,
...
impl<'a> Track<'a> {
    ...
    fn push_track_section(
        ...
        let speed_section = railjson.speed_sections.iter().find(|speed_section| {
            speed_section
                .track_ranges
                .iter()
                .any(|track_range| track_range.track == track_endpoint.track)
        });
        self.track_sections
            .push((track_section, direction, speed_section));
    ...
    fn length(&self) -> f64 {
        self.track_sections
            .iter()
            .map(|(track_section, _, _)| track_section.length)
            .sum()
    }
    ...
    fn get_track_section_at_distance(
        &self,
        distance: f64,
    ) -> Option<(&'a TrackSection, Direction, Option<&'a SpeedSection>, f64)> {
        let mut accumulated_distance = 0.0;

        for (track_section, direction, speed_section) in &self.track_sections {
            ...
            if accumulated_distance + ts_length >= distance {
                ...
                return Some((track_section, *direction, *speed_section, pk));
            }
    ...
    /// Get the highest speed limit along the track, if any.
    fn highest_speed_limit(&self) -> Option<Speed> {
        self.track_sections
            .iter()
            .filter_map(|(_, _, speed_section)| *speed_section)
    ...
    ///
    /// Due to a limitation of the current route generation, every signals must have it's equivalent in the opposite direction.
    /// But if we generate signals in both directions idependently, and then double each of them, we might end up with signals that
    ...
    /// But for it to work, we need to assume that the speed limit is the same in both directions and along the whole track.
    ...
    fn generate_block_signals(&self) -> Vec<Signal> {
        let speed_limit = self.highest_speed_limit();

        let (block_length, signal_type, jaune_cli) = if let Some(speed_limit) = speed_limit {
            if speed_limit.0 <= 160.0 / 3.6 {
                (
                    get_minimal_signaling_distance(speed_limit).unwrap(),
                    SignalType::Bal,
                    false,
                )
            } else if speed_limit.0 <= 220.0 / 3.6 {
                (
                    get_minimal_signaling_distance(Speed(160.0 / 3.6)).unwrap(),
                    SignalType::Bal,
                    true,
                )
            } else {
                (TVM_BLOCK_LENGTH, SignalType::Tvm, false)
            }
        } else {
            (DEFAULT_BLOCK_LENGTH, SignalType::Bal, false)
        };
        ...
        .flat_map(|(track_section, _direction, _speed_section, pk)| {
            [
                new_signal(
                    signal_type,
    ...
    fn generate_intersection_signals(&self) -> Vec<Signal> {
        ...
        if self.switch_at_begin
            && let Some((track_section, direction, _speed, pk)) =
        ...
        if self.switch_at_end
            && let Some((track_section, direction, _speed, pk)) = self
...
fn new_signal(
    ...
    let (sight_distance, signaling_system) = match signal_type {
        SignalType::Bal => (400.0, "BAL".to_string()),
...
mod tests {
    ...
    fn kvb_signal(
        ...
        let mut railjson =
            crate::osm_to_railjson::parse_osm("src/tests/kvb_signal.osm.pbf".into(), true).unwrap();
        super::generate_signals(&mut railjson);
