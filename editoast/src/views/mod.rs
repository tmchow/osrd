...
async fn authentication_validation_middleware(
    ...
    fn origin_user(
        ...
        identity: &str,
        header_name: &str,
    ) -> Option<(::authz::v2::Protected<Vec<Role>>, editoast_models::User)> {
        user.inspect(|user| {
        ...
        .map(|user| {
            (
                ::authz::v2::subject_roles(::authz::Subject::user(user.id)),
                user,
            )
        })
    }
    ...
    let (roles_prot, user) = conn
        .transaction(async move |conn| {
            ...
            Ok(match origin {
                ...
                Some((roles_prot, user)) => (roles_prot, Some(user)),
                ...
                None => {
                    ...
                    if let Some(origin) = authn.origin() {
                        ...
                        (::authz::v2::Protected::default(), None)
                    }
                }
            })
        })
