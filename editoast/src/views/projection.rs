...
fn extract_curve_for_invalid_train_with_sim<T: TrainScheduleLike>(
    ...
    // draw the final straight segment from schedule data alone. We cut the
    // simulation curve at the last scheduled position, compute the time
    // offset there, then re-apply the remaining simulated points with that
    // offset to keep the curve continuous.
    ...
    if last_scheduled_position != path.length {
        ...
        let nb_sim_points_to_skip = find_index_upper(&sim_curve.positions, last_scheduled_position);
        sim_curve
            .positions
            .into_iter()
            .zip(sim_curve.times)
            .skip(nb_sim_points_to_skip)
