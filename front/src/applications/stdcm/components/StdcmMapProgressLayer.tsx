...
const ANIMATIONS = ANIMATION_STEPS.reduce(
  ...
  [] as Array<{
    ...
    pointToGeojson: (point: AnimatedPoint, elapsedTime: number) => Feature<Point>;
  }>
);
...
const StdcmMapProgressLayer = () => {
  const [points, setPoints] = useState<AnimatedPoint[]>([]);

  /**
   * By calling the setPoint and changing the reference at every frame, the map is rerendered automatically by the react lifecycle.
   * I tried to do the same with a map.redraw (or similar), but it does'nt work well enough (animation are not smooth at all)
   */
  useEffect(() => {
    const timeoutId = setInterval(() => {
      setPoints((prev) =>
        Math.random() > 0.95
          ? [...prev, { lng: 45.6105 + 0.1 * prev.length, lat: 5.0259, timestamp: Date.now() }]
          : [...prev]
      );
    }, 0);
  ...
  // Compute the geojson from points
  ...
  return (
    <Source id="stdcm-algo-progress" type="geojson" data={geojson}>
      <OrderedLayer
