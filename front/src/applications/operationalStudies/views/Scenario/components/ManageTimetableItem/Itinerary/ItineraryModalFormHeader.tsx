...
import type { ItineraryModalContext } from './ItineraryModal';

type ItineraryModalFormHeaderProps = {
  modalContext: ItineraryModalContext;
  onModalContextChange: (context: ItineraryModalContext) => void;
...
const ItineraryModalFormHeader = ({
  ...
  const categoryWarningMessage = useMemo(() => {
    if (!rollingStock || !modalContext.category) return undefined;

    const isMismatch = isMainCategory(modalContext.category)
      ? modalContext.category.main_category !== rollingStock.primary_category &&
        !rollingStock.other_categories.includes(modalContext.category.main_category)
      : currentSubCategory?.main_category !== rollingStock.primary_category;

    return isMismatch ? t('categoryMismatch') : undefined;
  }, [rollingStock, modalContext.category, currentSubCategory, t]);
        ...
        <div className="category-select">
          <Select
            ...
            value={categoryOptions.find((option) =>
              isEqual(option.category, modalContext.category)
            )}
      ...
      <div className="rolling-stock-cc-name-row">
        ...
        <div className="composition-code-select">
          <Select
            ...
            value={modalContext.speedLimitTag ?? ''}
        ...
        <div className="train-name-input">
          <Input
            ...
            value={modalContext.name}
            title={modalContext.name}
            onChange={(e) =>
              onModalContextChange({
                ...modalContext,
                name: e.target.value,
